...
const ShowEmbeds = getUserSettingLazy<boolean>(
    "textAndImages",
    "renderEmbeds",
)!;
...
export function detectClient(): clientData {
    ...
    if (IS_EQUIBOP) {
        const equibopGitHash = tryOrElse(
            () => VesktopNative.app.getGitHash?.(),
            null,
        );
        const spoofInfo = tryOrElse(
            () => VesktopNative.app.getPlatformSpoofInfo?.(),
            null,
        );
        const isDevBuild = tryOrElse(
            () => VesktopNative.app.isDevBuild?.(),
            false,
        );
...
async function generateDebugInfoMessage() {
    ...
    const spoofInfo = IS_EQUIBOP
        ? tryOrElse(() => VesktopNative.app.getPlatformSpoofInfo?.(), null)
        : null;
    ...
    const info = {
        Equicord:
            `v${VERSION} • [${gitHashShort}](<https://github.com/BricksAreCool/mycord/commit/${gitHash}>)` +
    ...
    if (
        isPluginEnabled(customIdle.name) &&
        customIdle.settings.store.idleTimeout === 0
    ) {
    ...
    const commonIssues = {
        ...
        "Link Embeds Disabled": tryOrElse(
            () => !ShowEmbeds.getSetting(),
            false,
        ),
        "Equicord DevBuild": !IS_STANDALONE,
        "Equibop DevBuild":
            IS_EQUIBOP &&
            tryOrElse(() => VesktopNative.app.isDevBuild?.(), false),
...
async function uploadPluginListFile(
    channelId: string,
    fileContent: string,
    filename: string,
) {
    ...
    const upload = new CloudUploader(
        { file, platform: CloudUploadPlatform.WEB },
        channelId,
    );
...
function generatePluginList() {
    ...
    const enabledStockPlugins = enabledPlugins
        .filter((p) => !PluginMeta[p].userPlugin)
        .sort();
    const enabledUserPlugins = enabledPlugins
        .filter((p) => PluginMeta[p].userPlugin)
        .sort();
        ...
        Alerts.show({
            ...
            body: (
                <div>
                    ...
                    <Paragraph>
                        You have more than 100 plugins enabled.
                    </Paragraph>
                    <Paragraph>
                        Due to the sheer amount of plugins, you may not receive
                        support.
                    </Paragraph>
                    <Paragraph>
                        Your issue is likely caused by plugin conflicts.
                    </Paragraph>
                    <Paragraph>
                        Please consider disabling some plugins to troubleshoot.
                    </Paragraph>
                    <Paragraph className={Margins.top8}>
                        Your plugin list will be sent as a text file.
                    </Paragraph>
                </div>
            ),
        });
...
function DevBuildConfirmModal(props: RenderModalProps) {
    ...
    return (
        ...
        >
            <div>
                <Paragraph>
                    You are using a custom build of Equicord, which we do not
                    provide support for!
                </Paragraph>
                ...
                <Text variant="text-md/bold" className={Margins.top8}>
                    You will be banned from receiving support if you ignore this
                    rule.
                </Text>
            </div>
        </ConfirmModal>
...
export default definePlugin({
    ...
    commands: [
        {
            ...
            // @ts-ignore
            predicate: (ctx) =>
                isAnyPluginDev(UserStore.getCurrentUser()?.id) ||
                isEquicordGuild(ctx?.guild?.id, true),
            execute: async () => ({
        ...
        },
        {
            ...
            execute: async () => {
                ...
                if (typeof pluginList === "string") {
                    ...
                    pluginList &&
                    typeof pluginList === "object" &&
                    pluginList.uploadFile
                ) {
                    try {
                        ...
                        new Logger("SupportHelper").error(
                            "Failed to upload plugin list:",
                            e,
                        );
                        return {
                            content:
                                "Failed to upload plugin list file. Please try again.",
                        };
                    }
                }
    ...
    flux: {
        async CHANNEL_SELECT({ channelId }) {
                        ...
                        >
                            <div>
                                <Paragraph>
                                    ...
                                    Equicord! Chances are, your issue is already
                                    fixed.
                                </Paragraph>
                                <Paragraph className={Margins.top8}>
                                ...
                                <Paragraph className={Margins.top8}>
                                    If you know what you're doing or cannot
                                    update, you can dismiss this prompt.
                                </Paragraph>
                            </div>
                        </ConfirmModal>
                    ));
            ...
            const roles =
                GuildMemberStore.getSelfMember(VC_GUILD_ID)?.roles ||
                GuildMemberStore.getSelfMember(GUILD_ID)?.roles;
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                openModal((props) => (
                    ...
                    >
                        <div>
                            <Paragraph>
                                You are using an externally updated Equicord
    ...
    renderMessageAccessory(props) {
                        ...
                        onClick={async () => {
                            ...
                            if (typeof pluginList === "string") {
                                ...
                                try {
                                    ...
                                    showToast(
                                        "Plugin list uploaded successfully!",
                                        Toasts.Type.SUCCESS,
                                    );
                                } catch (e) {
                                    new Logger("SupportHelper").error(
        ...
        if (
            equicordSupport ||
            isSupportChannel(props.channel.id) ||
            isKnownIssuesCategory(props.channel.parent_id)
        ) {
        ...
        ({ channel }) => {
            ...
            return (
                <Card variant="warning" className={Margins.top8} defaultPadding>
                    Please do not private message Equicord & Vencord plugin
                    developers for support!
                    <br />
                    Instead, use the support channel:{" "}
                    {Parser.parse(
                        "https://discord.com/channels/1173279886065029291/1297590739911573585",
                    )}
